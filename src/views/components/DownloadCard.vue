...
<script>
	...
	export default {
		...
		methods:{
			thetitle(url,i){
				return this.titleid ?this.image.title + "." + i + "." + url.split(".")[url.split(".").length-1] : url.split("/")[url.split("/").length-1].split("?")[0]
			},
			IsPC() {
			...
			AriadownloadAll(){
				...
				if(this.image.type=="ugoira"){
					...
					for(var i = 0; i < imgs.length; i++){
						...
						var params = [
									[url], {'header':["Referer:https://www.pixiv.net/"],'out':this.thetitle(url,i)}
								]
						if(token){
						...
						this.axios
							...
							.then((response, state) => {
								...
								handle(response,data).then((pics) => {
									...
									gif.addFrame(pics[0][0], {delay: pics[0][1]});
									for(var j = 1; j < pics.length; j++){										
										gif.addFrame(pics[j][0], {delay: pics[j][1]});
									}
